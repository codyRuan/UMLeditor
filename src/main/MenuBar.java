...
//			}
			...
			if(c.getSelectedObj() != null) {
				if(!c.getSelectedObj().getGroup().isEmpty()) {
					b = c.getSelectedObj();		
//					group = b.getGroup().get(0).first;
				}
			}
			else if(c.getNumsOfGroup() == 1 && 
					!c.getSelectList().get(0).getGroup().isEmpty()) {
				b = c.getSelectList().get(0);
			}
			else
				return;
			
			for(BaseObject obj : b.getGroup().get(b.getGroup().size()-1).second) {
				obj.getGroup().remove(obj.getGroup().size()-1);
			}
			c.hideAll();
			
//			
//			if(!c.getSelectedObj().getGroup().isEmpty() 
//					|| (c.getSelectList().size() == 1 && !c.getSelectList().get(0).getGroup().isEmpty())) {
//				int group = 0;
//				if(!c.getSelectedObj().getGroup().isEmpty()) {
//					b = c.getSelectedObj();		
////					group = b.getGroup().get(0).first;
...
////					group = b.getGroup().get(0).first;
//				}
////				ArrayList<BaseObject> objList = b.getGroup().get(0).second;
//				for(BaseObject obj : b.getGroup().get(b.getGroup().size()-1).second) {
//					obj.getGroup().remove(obj.getGroup().size()-1);
//				}
//				c.hideAll();
